		...
		$(function(){
	     ...
	     var urlajax = '/api/order/show/'+order_id+'?openid='+openId;
			$.get( urlajax,function(data){
				var data = data.data; 
				var orderItem = data.foods.data;
				console.log(orderItem);
				for(var i=0;i<orderItem.length;i++){
					var price = parseFloat(orderItem[i].price);
					var original = parseFloat(orderItem[i].original_price);
					var li = document.createElement('li');
					li.className = "mui-table-view-cell mui-media";
					li.innerHTML = '<a href="javascript:;"><img class="mui-media-object mui-pull-left" src='+orderItem[i].img+'><div class="w-box"><div class="w-menu-left"><p class="menu-name">'+orderItem[i].name+'</p><small class="menu-address">'+orderItem[i].canteen+'</small><p class="menu-number"><span>月售:'+orderItem[i].sold+'</span></p><p class="menu-footer"><span class="vule-icon">￥</span><span class="vue-number">'+price+'</span>&nbsp;&nbsp;&nbsp;<span class="origin-value">原价:'+original+'元</span></p></div><div class="w-menu-right"><div class="love-icon" data-id='+orderItem[i].id+'><span class="mui-icon iconfont dianzan105"></span></div><div class="add-icon-a"><span class="num-menu-num">份数：'+orderItem[i].quantity+'</span></div></div></div></a>';
				...
				div.innerHTML  = '<ul class="w-cash-all mui-table-view"><li class="mui-table-view-cell">合计总额:<span class="mui-pull-right">'+total+'元(含服务费)</span></li></ul><ul class="w-home-tab mui-table-view"><li class="mui-table-view-cell">订单编号：'+data.order_no+'<span class="w-hold mui-pull-right">'+data.status+'</span></li><li class="mui-table-view-cell"><div class="telShow">联系电话：'+data.orderer.phone+'<a href="tel:15586879654"></a></div></li><li class="mui-table-view-cell">联系姓名：'+data.orderer.name+'</li><li class="mui-table-view-cell">配送地址：'+data.address+'</li></ul>';
